...
function burger_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'burger' ),
        'next_string'     => esc_html__( '&raquo;', 'burger' ),
        'before_output'   => '<div class="burger-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'burger_pagination_defaults', $defaults )
    );
