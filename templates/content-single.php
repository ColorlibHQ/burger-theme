...
<div id="<?php the_ID(); ?>" <?php post_class('single-post-area col-12 blog-post-details'); ?>>
	<div class="single-blog">
		 ...
		 * Post social share
		 * @Hook  burger_blog_posts_content
		 *
		 * @Hooked burger_blog_posts_content_cb
		 * 
		 *
		 */
		do_action( 'burger_blog_posts_content' );
		
		/**
		 * Blog post share content
		 * Post social share
		 * @Hook  burger_blog_posts_share
		 *
		 * @Hooked burger_blog_posts_share_cb
		 * 
		 *
		 */
		do_action( 'burger_blog_posts_share' );

		?>   
	</div>
	<?php 
	do_action('burger_blog_single_meta');
	// comment template.
