...
function burger_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'burger' ) . '</span></li>';
...
// Blog categories
function burger_featured_post_cat(){
...
// social media
if ( ! function_exists( 'burger_social' ) ) {
	function burger_social( $args = array() ) {
		...
		$url = burger_opt( 'burger_social_url' );
...
// header cart count
function burger_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( BURGER_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'burger' ); ?>">
...
function burger_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'burger_contact7_form_content', 10, 2 );